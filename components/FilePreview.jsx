...
const FileReview = ({ file, onRemove, uploadingFile, setUploadingFile }) => {
  if (!file) return null;
  ...
  const uploadFile = async (e) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/upload_zip_file/', formData);
      ...
      {uploadingFile ?
        <>
          <div className="flex flex-col items-center justify-center space-y-4 p-6 text-center w-full">
            <div className="w-10 h-10 border-4 border-solid border-orange-600 border-t-transparent rounded-full animate-spin"></div>

            <p className="text-sm font-bold text-gray-600">
              Uploading file. Please do not move away from this page!
            </p>

            <div className="w-full max-w-sm bg-gray-200 rounded-full h-3 overflow-hidden">
              <div
                className="bg-orange-600 h-full rounded-full animate-pulse transition-all duration-500"
                style={{ width: '35%' }}
              ></div>
            </div>

            <button
              className="bg-red-600 text-white py-2 px-6 rounded-full font-semibold hover:bg-orange-700 transition"
